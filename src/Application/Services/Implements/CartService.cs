...
namespace TiendaUCN.src.Application.Services.Implements
{
    public class CartService : ICartService
    {
        ...
        )
        {
            ...
            {
                Log.Information(
                    "El producto con ID {ProductId} no tiene suficiente stock.",
                    productId
                );
            ...
            if (existingProduct != null)
            {
                ...
                Log.Information(
                    "Actualizando cantidad del producto {ProductId} en el carrito.",
                    productId
                );
            }
            else
            {
                ...
                Log.Information(
                    "Nuevo producto agregado al carrito. ProductId: {ProductId}, Cantidad: {Quantity}",
                    productId,
                    quantity
                );
            }
        ...
        public async Task<CartDTO> ClearAsync(string buyerId, int? userId = null)
        {
            ...
            {
                Log.Information(
                    "El carrito no existe para el comprador especificado {BuyerId}.",
                    buyerId
                );
        ...
        public async Task<CartDTO> CreateOrGetAsync(string buyerId, int? userId = null)
        {
            ...
            if (cart == null)
            {
                if (userId.HasValue)
                {
                    ...
                    {
                        Log.Information(
                            "Se encontró carrito existente del usuario durante CreateOrGet. UserId: {UserId}",
                            userId
                        );
                ...
                cart = await _cartRepository.CreateAsync(buyerId, userId);
                Log.Information(
                    "Nuevo carrito creado para buyerId: {BuyerId}, userId: {UserId}",
                    buyerId,
                    userId
                );
            }
        ...
        )
        {
            ...
            {
                Log.Information(
                    "El artículo no existe en el carrito para el comprador especificado {BuyerId}.",
                    buyerId
                );
        ...
        public async Task AssociateWithUserAsync(string buyerId, int userId)
        {
            ...
            if (existingUserCart != null)
            {
                ...
                Log.Information(
                    "Carritos fusionados. Carrito anónimo {AnonymousCartId} → Carrito usuario {UserCartId}",
                    cart.Id,
                    existingUserCart.Id
                );
            }
            else
            {
                ...
                await _cartRepository.UpdateAsync(cart);
                Log.Information(
                    "Carrito anónimo asociado con usuario. BuyerId: {BuyerId} → UserId: {UserId}",
                    buyerId,
                    userId
                );
            }
        ...
        )
        {
            ...
            {
                Log.Information(
                    "El producto no existe para el ID especificado {ProductId}.",
                    productId
                );
            ...
            {
                Log.Information(
                    "El producto con ID {ProductId} no tiene suficiente stock para la cantidad solicitada.",
                    productId
                );
        ...
        private static void RecalculateCartTotals(Cart cart)
        {
            ...
            Log.Information(
                "Totales recalculados. SubTotal: {SubTotal}, Total: {Total}",
                cart.SubTotal,
                cart.Total
            );
        }
        ...
        public async Task<CartDTO> CheckoutAsync(string buyerId, int? userId)
        {
            ...
            {
                Log.Information(
                    "El carrito está vacío para el comprador especificado {BuyerId}.",
                    buyerId
                );
            ...
            foreach (var item in cart.CartItems.ToList())
            {
                ...
                {
                    Log.Information(
                        "Ajustando cantidad del producto {ProductId} con cantidad actualizada {NewQuantity}",
                        item.ProductId,
                        productStock
                    );
            ...
            if (hasChanges)
            {
                ...
                Log.Information(
                    "Carrito actualizado tras checkout. ItemsEliminados: {RemovedCount}, ItemsActualizados: {UpdatedCount}",
                    itemsToRemove.Count,
                    itemsToUpdate.Count
                );
            }
            ...
            {
                Log.Information(
                    "No se requirieron ajustes en el carrito durante checkout. CartId: {CartId}",
                    cart.Id
                );
            }
